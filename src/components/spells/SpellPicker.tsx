...
export function SpellPicker({ prepared, onToggle, onClose }: SpellPickerProps) {
  const t = useTheme();
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 70, background: 'rgba(4,6,10,.6)', backdropFilter: 'blur(6px)', display: 'grid', placeItems: 'center', padding: 16 }}>
      <div onClick={(e) => e.stopPropagation()} className="fv-panel animate-popIn" style={{ width: '100%', maxWidth: 520, maxHeight: '82vh', display: 'flex', flexDirection: 'column', padding: 20, border: '1px solid var(--gold)' }}>
        ...
        <div className="fv-scroll-thin" style={{ overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 7 }}>
          {list.map((sp) => {
            ...
            return (
              <button
                ...
                style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 11, textAlign: 'left', padding: '10px 13px', borderRadius: 11, border: '1px solid ' + (on ? t.gold : t.line), background: on ? hexA(t.gold, 0.1) : 'rgba(0,0,0,.26)' }}
              >
